...
TEST(ListConstIterators, Test3) {
  s21::list<int> A = {12, 22, 32};
  s21::list<int>::const_iterator it = A.cbegin();
  EXPECT_EQ(22, *(++it));
}

TEST(ListConstIterators, Test4) {
  s21::list<int> A = {12, 22, 32};
  s21::list<int>::const_iterator it = A.cend();
  EXPECT_EQ(32, *(--it));
}

TEST(ListConstIterators, Test5) {
  s21::list<int> A = {12, 22, 32};
  s21::list<int>::const_iterator it = A.cend();
  EXPECT_EQ(12, *(++it));
}

TEST(ListConstIterators, Test6) {
  s21::list<int> A = {12, 22, 32};
  s21::list<int>::const_iterator it = A.cend();
  it--;
  EXPECT_EQ(32, *it);
...
TEST(ListConstIterators, Test7) {
  s21::list<int> A = {12, 22, 32};
  s21::list<int>::const_iterator it_1 = A.cend();
  s21::list<int>::const_iterator it_2 = A.cend();
...
TEST(ListElementAccess, Back4) {
  const s21::list<int> A = {10, 20, 30};
  s21::list<int>::const_iterator itr = A.cend();
  EXPECT_EQ(*(--itr), A.back());
}
...
TEST(ListModifiers, EraseLastElement) {
  s21::list<int> list = {1};
  s21::list<int> expected;
...
TEST(ListMergeTest, IteratorAfterMerge) {
  s21::list<int> list1 = {1, 3, 5};
  s21::list<int> list2 = {2, 4, 6};
...
TEST(ListMergeTest, EndIteratorAfterMerge) {
  s21::list<int> list1 = {1, 3, 5};
  s21::list<int> list2 = {2, 4, 6};
...
// Тест 3: Уникальные элементы, ничего не должно измениться
...
// Тест 4: Последовательные дубликаты, должны быть удалены
...
// Тест 5: Непоследовательные дубликаты, ничего не должно измениться
TEST(ListOperations, UniqueNonConsecutiveDuplicates) {
  s21::list<int> list = {1, 2, 3, 1, 4, 5};
  s21::list<int> expected = {1, 2, 3, 1, 4, 5};
  list.unique();
  EXPECT_EQ(list, expected);
}

// Тест 5: Непоследовательные дубликаты, ничего не должно измениться
TEST(ListOperations, UniqueListIteratorCompare) {
  s21::list<int> list = {1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 4, 4, 5};
  s21::list<int> expected = {1, 2, 3, 4, 5};

  s21::list<int>::iterator itr_list = list.begin();
  s21::list<int>::iterator itr_expected = expected.begin();

  list.unique();

  for (auto i = expected.size(); i > 0; --i, ++itr_list, ++itr_expected) {
    EXPECT_TRUE(*itr_list == *itr_expected);
  }

  EXPECT_EQ(list, expected);
}

// Тест 6: Пустой список с пользовательским типом данных
...
// Тест 7: Пользовательский тип данных, несколько дубликатов
...
// Тест 8: Пользовательский тип данных, все элементы уникальны
...
// Тест 9: Пользовательский тип данных, дубликаты в конце списка
TEST(ListOperations, UniqueCustomTypeDuplicatesAtEnd) {
  s21::list<std::string> list = {"apple", "banana", "orange", "banana",
                                 "apple"};
  s21::list<std::string> expected = {"apple", "banana", "orange", "banana",
                                     "apple"};
  list.unique();
  EXPECT_EQ(list, expected);
}

// Тест 10: Пользовательский тип данных, дубликаты в начале списка
...
TEST(ListSort, IteratorsRemainValid) {
  s21::list<int> list = {5, 4, 3, 2, 1};
  s21::list<int> expected = {1, 2, 3, 4, 5};
  auto it_begin = list.begin();  // 5
  auto it_end = --list.end();    // 1
  list.sort();
  EXPECT_EQ(list, expected);
  EXPECT_EQ(list.size(), expected.size());
  EXPECT_EQ(it_begin, --list.end());
  EXPECT_EQ(it_end, list.begin());
}
